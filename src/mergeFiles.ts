  ...
  return new Promise((resolve, reject) => {
    merge
      ...
      .on("start", function (commandLine) {
        ...
        console.log("MergeFiles Spawned Ffmpeg with command: " + commandLine);
      })
      .on("progress", async function (progress) {
        ...
        const percent = Math.round(
          Math.max(0, (timeMillis / durationMillis) * 100)
        );
        ...
        if (scaledPercent !== previousScaledPercent) {
          ...
          console.log("MergeFiles Progress:", scaledPercent);
      ...
      .on("error", function (err) {
        console.log("MergeFiles Error:", err);
        return reject(err);
      })
      .pipe(writeStream);
  });
